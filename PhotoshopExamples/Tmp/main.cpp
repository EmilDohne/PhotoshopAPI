	...
	int width, int height)
{
	...
	for (const auto& [channel_id, data] : channel_map) {
		if (data.size() != static_cast<size_t>(width * height)) {
			throw std::runtime_error("Channel data size does not match image dimensions.");
		}

		channel_names.push_back(channel_id_to_string(channel_id)); // Convert channel ID to string
		channel_ptrs.push_back(data.data());
	}
	...
	// Write the image
	if (!out->write_image(TypeDesc::UINT8, channel_ptrs[0])) {
		throw std::runtime_error("Failed to write image: " + out->geterror());
	}
...
int main()
{
	...
	LayeredFile<bpp8_t> file = LayeredFile<bpp8_t>::read("C:/Users/emild/Desktop/linkedlayers/warp/warptest_single.psd");

	auto layer_ptr = find_layer_as<bpp8_t, SmartObjectLayer>("WarpQuilt", file);
	
	auto mesh = layer_ptr->warp().surface().mesh(50, 50, layer_ptr->warp().affine_transform(), layer_ptr->warp().non_affine_transform());
	...
	write_to_disk(data, "C:/Users/emild/Desktop/linkedlayers/warp/warpmesh.jpg", buffer.width, buffer.height);

	// auto orig_image_data = layer_ptr->original_image_data();
	// auto image_data = layer_ptr->get_image_data();

	// write_to_disk(orig_image_data, "C:/Users/emild/Desktop/linkedlayers/warp/original.jpg", layer_ptr->original_width(), layer_ptr->original_height());
	// write_to_disk(image_data, "C:/Users/emild/Desktop/linkedlayers/warp/warped.jpg", layer_ptr->width(), layer_ptr->height());
