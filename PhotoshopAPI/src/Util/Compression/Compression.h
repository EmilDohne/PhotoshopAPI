...
template <typename T>
inline std::vector<T> DecompressData(File& document, const Enum::Compression& compression, File& document, const FileHeader& header, const uint32_t width, const uint32_t height, const uint64_t compressedSize)
{
	switch (compression)
	{
