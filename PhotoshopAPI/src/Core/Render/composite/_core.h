...
Core of the compositing code, used to dispatch the individual kernels onto the pixels of two layers. The functions
here already expect that e.g. the alpha has been computed and special layer effects such as clipping masks or
layer knock outs have already been applied.
...
namespace composite
{
	...
	{
		static const std::unordered_map<Enum::BlendMode, KernelFunc<_Precision>> blend_map = {
			{ Enum::BlendMode::Normal,      kernel::normal<_Precision> },
			{ Enum::BlendMode::Multiply,    kernel::multiply<_Precision> },
			{ Enum::BlendMode::Screen,      kernel::screen<_Precision> },
			{ Enum::BlendMode::Overlay,     kernel::overlay<_Precision> },
			{ Enum::BlendMode::Darken,      kernel::darken<_Precision> },
			{ Enum::BlendMode::Lighten,     kernel::lighten<_Precision> },
			{ Enum::BlendMode::ColorDodge,  kernel::color_dodge<_Precision> },
			{ Enum::BlendMode::ColorBurn,   kernel::color_burn<_Precision> },
			{ Enum::BlendMode::LinearDodge, kernel::linear_dodge<_Precision> },
			{ Enum::BlendMode::LinearBurn,  kernel::linear_burn<_Precision> }
		};

		auto it = blend_map.find(blend_mode);
		if (it == blend_map.end())
		{
		...
		// Dispatch the correct blending kernel
		impl::rgb::iter_apply<T, _Precision>(canvas, layer, it->second);
	}
