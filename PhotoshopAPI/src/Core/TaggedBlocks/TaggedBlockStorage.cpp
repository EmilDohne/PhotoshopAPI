...
template std::shared_ptr<OcioTaggedBlock>				TaggedBlockStorage::getTaggedBlockView(const Enum::TaggedBlockKey key) const;
...
template std::shared_ptr<OcioTaggedBlock>				TaggedBlockStorage::getTaggedBlockView() const;
...
template std::vector<std::shared_ptr<OcioTaggedBlock>>				TaggedBlockStorage::get_tagged_blocks() const;
...
const std::shared_ptr<TaggedBlock> TaggedBlockStorage::readTaggedBlock(File& document, const FileHeader& header, ProgressCallback& callback, const uint16_t padding)
{
	...
	if (taggedBlock.has_value())
	{
		...
		else if (taggedBlock.value() == Enum::TaggedBlockKey::ocioData)
		{
			auto ocioBlock = std::make_shared<OcioTaggedBlock>();
			ocioBlock->read(document, offset, taggedBlock.value(), signature, padding);
			this->m_TaggedBlocks.push_back(ocioBlock);
			return ocioBlock;
		}
		else
		{
