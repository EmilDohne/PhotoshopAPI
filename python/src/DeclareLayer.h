...
void declareLayer(py::module& m, const std::string& extension) {
    ...
    py::class_<Class, std::shared_ptr<Class>> layer(m, className.c_str(), py::dynamic_attr(), py::buffer_protocol(), R"pbdoc(
        ...
        This class exposes the Layer.Params class which is expected to be passed to the constructor of
        the other classes
    ...
    layer.def_readwrite("layer_mask", &Class::m_LayerMask);
