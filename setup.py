# This setup.py is mostly copied from the cmake_example provided by pybind
...
version_pattern = r'project\s*\(\s*"\w*"\s+VERSION\s+(\d+(?:\.\d+){0,2})'
# Search for the version string using regex
match = re.search(version_pattern, cmake_content)
# Extract version string if found
if match:
    __version__ = match.group(1)
    print("Version extracted from CMakeLists.txt:", __version__)
else:
    print("Version string not found in CMakeLists.txt")
    __version__ = "0.0.0"
...
setup(
    ...
    package_data={"psapi-stubs": ["python/psapi-stubs/*.pyi", "python/psapi-stubs/py.typed"]},
    zip_safe=False,
    install_requires= [
        "numpy>=1.26"
    ],
